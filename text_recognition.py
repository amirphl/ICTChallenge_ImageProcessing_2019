...
passed = []
my_size = 0
for x in range(0, m_height, args['iterh']):
    temp = []
    for r in results:
        if x <= r[0][1] < x + args['iterh'] and r not in passed:
            temp.append(r)
            passed.append(r)
    my_size += len(temp)
    final_arrays.append(temp)

assert my_size == len(results)

# loop over the results
for element in final_arrays:
